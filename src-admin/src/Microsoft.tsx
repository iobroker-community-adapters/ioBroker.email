...
type MicrosoftState = ConfigGenericState & {
    url: string;
    accessTokens: string;
    success: boolean;
    blocked: boolean;
...
class Microsoft extends ConfigGeneric<ConfigGenericProps, MicrosoftState> {
    ...
    onMessage = (event: MessageEvent): void => {
        ...
        if (
            (typeof event.data === 'string' && event.data.startsWith('microsoft-authentication:')) ||
            (typeof (event as any).message === 'string' &&
                (event as any).message.startsWith('microsoft-authentication:'))
        ) {
    ...
    onOpenUrl(): void {
        this.authWindow = window.open(this.state.url, 'ioBrokerMicrosoftAuth');
    ...
    renderItem(): React.JSX.Element {
        ...
        return (
            <div style={{ width: '100%', margin: '0 0 1rem 0' }}>
                ...
                >
                    {I18n.t(this.state.accessTokens ? 'Renew Microsoft Access' : 'Get Microsoft Access')}
                </Button>
                {this.state.blocked ? (
