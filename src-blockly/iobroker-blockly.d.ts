 ...
 */
import type { Block, FieldCheckbox, FieldDropdown, FieldImage } from 'blockly/core';

/**
 * A block definition. Beside `init` it may carry mutation callbacks and helpers of its own, so the
 * members are left open. `init` is declared without a `this` type on purpose - the block annotates
...
/**
 * The JavaScript code generator. Only the members the block file actually uses are declared - the
