/**
 * The words of the email block.
 *
 * They are bundled into `admin/blockly.js` at build time, because the editor loads that file as a
 * classic script and `Blockly.Words` must be filled before the block is registered - there is no
...
/** Fills `Blockly.Words` with everything the email block needs */
export function installWords(): void {
    ...
    // A language that does not have a word simply falls back to English.
    ...
    // A link, not a word - it must not be handed to the translator. The hand-written table spelled
    // the same URL out once per language.
